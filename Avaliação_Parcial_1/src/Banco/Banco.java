...
// instruções do método a ser desenvolvido
        ...
        while(!origemValida) {
            mostrarInfo(contas);
            System.out.println("Qual o a sua conta ?(0 a " + (contas.length - 1));
            indiceContaOrigem = entrada.nextInt();
            if (indiceContaOrigem >= 0  && indiceContaOrigem < contas.length) {
                origemValida = true;
            } else {
                System.out.println("Índice de origem inválido!");
            }
        }
        ...
        while(!destinoValido) {
            mostrarInfo(contas);
            System.out.print("A transferencia será efetuado para qual cliente?(0 a " + (contas.length - 1) + ". Exceto " + indiceContaOrigem + "!)");
            indiceContaDestino = entrada.nextInt();
            if (indiceContaDestino >= 0 && indiceContaDestino < contas.length && indiceContaOrigem != indiceContaDestino) {
                destinoValido = true;
            } else {
                System.out.println("Índice de cliente inválido!");
            }
